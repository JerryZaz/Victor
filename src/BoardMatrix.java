...
public class BoardMatrix extends Matrix
{
    ...
    public void findPlayables() //getLowestEmptyIndex
    {
    ...
    /**
     * Checks the board for the opponent's move and checks for the threats
     * that are influenced by it, increasing the threat level of those
     * threats that contain this move.
     * 
     * @param check Vicky sends the backup board to check for differences
     * @param threats Receives the threats from Vicky
     */
    public void checkOpponentMove(BoardMatrix check, ArrayList<Threat> threats)
    {
        for(int i = 0; i < board[0].length; i++){
            for(int j = 0; j < board.length; j++){
                if(board[j][i] != check.getChar(j,i)){
                    Problem move = new Problem(j,i);
                    for(int x = 0; x < threats.size(); x++){
                        if(threats.get(x).isActive()){
                            if(threats.get(x).containsProblem(move)){
                                int threatLevel = threats.get(x).getThreatLevel();
                                if(threatLevel < 4){
                                    threatLevel+=1;
                                    threats.get(x).setThreatLevel(threatLevel);
                                }
                            }
                        }
                    }
